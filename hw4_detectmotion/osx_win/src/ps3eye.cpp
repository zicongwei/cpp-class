#ifndef __PS3EYE_CS__
#define __PS3EYE_CS__
...
int run_camera(Context& context, Scalar colour)
{
	int width = 640;
	int height = 480;
	int fps = 60;
	...
	char title[256];
	sprintf(title, "Input%dx%d@%d\n", ctx.eye->getWidth(), ctx.eye->getHeight(), ctx.eye->getFrameRate());

    namedWindow("Output", WINDOW_AUTOSIZE ); //OPENCV window
    ...
    background.set("nmixtures", 3);
    background.setBool("detectShadows", false);
    Point *prevMC = new Point();
	...
	while (ctx.running) {
        ...
        frame_function(context, colour, m, background, prevMC);
